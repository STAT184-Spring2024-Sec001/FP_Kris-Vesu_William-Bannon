...
We intend to analyze all sorts of relevant dating relating to the stars, including but not limited to: points, true shooting, assists, field goals, and rebounds. These statistics will help us analyze each players individual stats to determine who is the better player overall. All of these were already attributes of the original dataset, except true shooting, which was created by us and is discussed in further detail later in the document.
...
#In this code we read in the data from our file directory and truncate the string values of each players ages to just years and not days so that it might function for graphing purpose.
...
full_table <- rbind(lebron_data, jordan_data)
View(full_table)
...
```{r points}
ggplot() +
  geom_boxplot(data = full_table, aes(x = age, y = pts, color = name)) +
...
```{r true shooting}
ggplot() +
  geom_boxplot(data = full_table, aes(x = age, y = ts, color = name)) +
  scale_color_manual(values = c("red", "blue")) +
...
The following summary tables show off the amount of field goals both players have made, which is signified under the count column. For the rest of the columns, we can see the minimum and maximum amount efficiency, the first through fourth quintiles of each game that was efficient, the mean and median amount of efficiency and the standard deviation efficiency. Examining the data we can see that Lebron's true shooting percentage elevates with the field goals he takes and he manages to to best Jordan in plenty of categories, especially the median values.
